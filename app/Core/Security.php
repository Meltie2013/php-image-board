...
class Security
{
    ...
    /**
     * Generate a short-lived token bound to the current session and user.
     *
     * This is intended for future sensitive flows that need a token stronger
     * than a plain random string while still keeping the PHP session ID opaque.
     *
     * @param string $purpose Token purpose / namespace.
     * @param int $ttlSeconds Time-to-live in seconds.
     * @param array $claims Optional extra claims to embed.
     * @return string Signed token string.
     */
    public static function generateSessionBoundToken(string $purpose, int $ttlSeconds = 900, array $claims = []): string
    {
        return SessionManager::issueBoundToken($purpose, $ttlSeconds, $claims);
    }

    /**
     * Verify a short-lived token generated by generateSessionBoundToken().
     *
     ...
     */
    public static function verifySessionBoundToken(string $token, string $purpose): ?array
    {
        return SessionManager::verifyBoundToken($token, $purpose);
    }
